...
│       └── main.go
├── internal/
│   ├── crawl/                engine + Schema A (schema.sql)
│   │   ├── aimd.go
│   │   ├── client.go
│   │   ├── CONTRACT.md
│   │   ├── dedupe.go
...
│       ├── harrisfarm/
│       └── woolworths/
├── tui/                      live TUI + Schema B builders; commands stay uv run tui/...
│   ├── browser_mode.py
│   ├── build_api_db.py
│   ├── build_master.py
│   ├── canonical.py
...
├── deploy/
│   └── nightly.sh
├── data/                     generated: hyper_*.db, master.db, basketwise.db, warm/
├── example_requests/
│   ├── coles/
│   └── woolworths/
├── go.mod
...
```
store APIs
  -> ./hyperscrape -store coles|woolworths|aldi|harrisfarm
  -> data/hyper_<store>.db
  -> tui/build_master.py
  -> data/master.db
  -> tui/build_api_db.py
  -> data/basketwise.db
  -> rsync to hpsrv
  -> backend FastAPI at https://basket.taskglass.work/
```
...
Two schemas (see [Source-of-truth.md](../Source-of-truth.md) §2). This section is **Schema A** (crawl dumps). Schema B (`basketwise.db`) is built later by `tui/build_api_db.py`.
...
(create it once with `_can_possibly_delete/python-supabase/supabase/001_product_details.sql`).
...
the last good artifact rather than garbage. See `backend/README.md`.
