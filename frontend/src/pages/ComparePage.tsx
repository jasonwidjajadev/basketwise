...
export default function ComparePage() {
  ...
  const bestTotal = availableTotals.length
    ? Math.min(...availableTotals)
    : null

  const tie =
    availableTotals.length > 0 &&
    availableTotals.every(
      (t) => Math.abs(t - bestTotal) < 0.005,
    )
  ...
  const hasExcluded =
    !hasUnavailable &&
    data.recommended.excludedItems.length > 0

  return (
    <main className="w-full px-6 pt-9.5 pb-24 lg:px-8 xl:px-12 2xl:px-16">
      <div className="mx-auto w-full max-w-7xl">
        <h1 className="text-[32px] text-bw-ink">
        ...
        <div className="mt-6">
          {hasUnavailable && (
            <div className="animate-bw-fade-up mb-5">
              <UnavailableBanner>
                ...
                isn't available at any store right now — it's been
                left out of every total below.
              </UnavailableBanner>
            </div>
          )}
          ...
          {hasExcluded && (
            <div className="animate-bw-fade-up mb-5">
              <UnavailableBanner>
                <strong className="text-bw-ink">
                  {data.recommended.excludedItems
                    .map((p) => p.name)
                    .join(', ')}
                </strong>{' '}
                isn't carried by any store in your delivery area, so
                it's left out of the Recommended split entirely. See
                "Lowest possible price" to include it.
              </UnavailableBanner>
            </div>
          )}
              ...
              {cards.map((c, i) => (
                <OptionCard
                  ...
                  isBest={
                    c.available &&
                    !tie &&
                    Math.abs(c.option.total - bestTotal) <
                      0.005
                  }
                  whyOpen={showWhy}
                  onToggleWhy={() =>
