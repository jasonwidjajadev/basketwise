...
const BASE: string =
  (import.meta as unknown as { env?: Record<string, string> }).env
    ?.VITE_API_BASE ?? 'https://basket.taskglass.work'
...
export async function compare(
  ...
  if (!res.ok)
    throw new ApiError(res.status, `POST /compare failed: ${res.status}`)
...
export function formatUnitPrice(
  p: Pick<Product, 'unit_price' | 'unit_measure'>,
): string {
...
export function discountPercent(
  p: Pick<Product, 'min_price' | 'was_price'>,
): number | null {
  if (p.was_price == null || p.min_price == null || p.was_price <= p.min_price)
    return null
...
export function formatSize(
  p: Pick<Product, 'size_value' | 'size_unit'>,
): string {
